...
export const createUserListing = async (req, res) => {
    try {
        const { title, location, price, beds, baths, length, breadth, sqft, type, description, phone, googleMapLink, facing } = req.body;
        ...
        const requiredFields = ['title', 'location', 'price', 'sqft', 'type', 'description', 'phone'];
        if (type !== 'Plot') requiredFields.push('beds', 'baths');
...
export const updateUserListing = async (req, res) => {
    try {
        ...
        const updates = {
            ...
            ...(breadth !== undefined && { breadth: Number(breadth) }),
